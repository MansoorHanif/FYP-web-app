...
from flaskbb.pet.models import Pet

class PetWhoosheer(AbstractWhoosheer):
    models = [Pet]

    schema = whoosh.fields.Schema(
        pet_id=whoosh.fields.NUMERIC(stored=True, unique=True),
        owner_id=whoosh.fields.NUMERIC(stored=True),
        petname=whoosh.fields.TEXT()
    )

    @classmethod
    def update_pet(cls, writer, pet):
        writer.update_document(
            pet_id=pet.id,
            owner_id=pet.user.id,
            petname=text_type(pet.petname)
        )

    @classmethod
    def insert_pet(cls, writer, pet):
        writer.add_document(
            pet_id=pet.id,
