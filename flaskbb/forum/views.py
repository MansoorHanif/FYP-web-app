...
@forum.route("/petlist", methods=['GET', 'POST'])
def petlist():
    page = request.args.get('page', 1, type=int)
    sort_by = request.args.get('sort_by', 'reg_date')
    order_by = request.args.get('order_by', 'asc')
    ...
    if sort_by == 'reg_date':
        ...
        sort_obj = Pet.petname

    search_form = PetSearchForm()
    if search_form.validate():
        pets = search_form.get_results().\
            paginate(page, flaskbb_config['PETS_PER_PAGE'], False)
        return render_template("forum/petlist.html", pets=pets,
                               search_form=search_form)
    else:
        pets = Pet.query.order_by(order_func(sort_obj)).\
            paginate(page, flaskbb_config['PETS_PER_PAGE'], False)
        return render_template("forum/petlist.html", pets=pets,
                               search_form=search_form)
