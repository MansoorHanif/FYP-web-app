...
@user.route("/<username>/pets")
def view_all_pets(username):
    page = request.args.get("page", 1, type=int)
    user = User.query.filter_by(username=username).first_or_404()
    pets = user.all_pets(page, current_user)
    return render_template("user/all_pets.html", user=user, pets=pets)

@user.route("/<username>/reminders")
def view_all_reminders(username):
    page = request.args.get("page", 1, type=int)
    user = User.query.filter_by(username=username).first_or_404()
    reminders = user.all_reminders(page, current_user)
    return render_template("user/all_reminders.html", user=user, reminders=reminders)
