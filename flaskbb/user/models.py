...
class User(db.Model, UserMixin, CRUDMixin):
    ...
    pets = db.relationship("Pet", backref="user", cascade="all, delete-orphan" , lazy='dynamic')
    ...
    def all_pets(self, page, viewer):
        ...
        group_ids = [g.id for g in viewer.groups]
        pets = Pet.query.\
            filter(Pet.user_id == self.id,
                Forum.groups.any(Group.id.in_(group_ids))).\
            paginate(page, flaskbb_config['PETS_PER_PAGE'], False)
        return pets

    def all_reminders(self, page, viewer):
        
        group_ids = [g.id for g in viewer.groups]
        reminders = Reminder.query.\
            filter(Reminder.user_id == self.id,
                Forum.groups.any(Group.id.in_(group_ids))).\
            paginate(page, flaskbb_config['PETS_PER_PAGE'], False)
        return reminders
